...
//			//add code for popup menu if button style is tab and it's been right clicked.
//#ifdef PLUGINARCHITECTUREDLL1_EXPORTS
//			
...
//#endif // PLUGINARCHITECTUREDLL1_EXPORTS
//
...
LRESULT Theme::ListViewButtonProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
	{
		...
		default:
			...
			SendMessage(GetParent(hWnd), WM_PLUGINLVACTIVATE, wParam, cData->ButtonNumber);
			break;
		}
		break;
	}
	case WM_RBUTTONDOWN:
	{
		...
		std::vector<Theme::LVDropDownMenu>* vLVDDM = (std::vector<Theme::LVDropDownMenu>*)SendMessage(GetParent(hWnd), WM_GETLVDDM, NULL, NULL);


		auto vSearch = vLVDDM->begin();
		for(vSearch; vSearch != vLVDDM->end(); vSearch++)
		{			
			InsertMenu(popupMenu, NULL, MF_STRING, vSearch->uIDNewItem , vSearch->MenuName.c_str());
		}
