...
typedef Plugin* (WINAPI* f_CreateModule)();
typedef wchar_t* (WINAPI* f_GetPluginName)();
typedef wchar_t* (WINAPI* f_GetListViewName)();
typedef bool (WINAPI* f_GetListViewBool)();
typedef bool (WINAPI* f_GetImmediateActivation)();

std::vector<Plugin*> vPlugins;
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) //Main windows procedure
{
	...
	{
		//ClientData::ClientDataStrings* cData = (ClientData::ClientDataStrings*)lParam;
		//vPlugins[wParam]->Initialize(cData);
		((Winsock*)(mData->Winsock))->cHandler.ClientActivator((ClientData*)lParam, vPlugins[wParam]->usingListView);
	...
	case WM_PLUGININIT:
	{
		...
		vPlugins[wParam]->Initialize(lParam);
	}
	break;
	...
	case WM_CREATE:
	{
		...
		mData->HWND.Addons = Theme::CWindow(r, L"Segoe UI", L"CBUTTON", L"Add-Ons", WS_VISIBLE, 742, 34, 45, 16, hWnd, (HMENU)IDB_ADDONS);
	...
	case WM_COMMAND:
	{
		...
		case IDB_ADDONS:
		{
			...
			ofn.lpstrFile = (LPWSTR)file.data();
			ofn.nMaxFile = 256;
			...
			std::wstring wstr;
			//ss << wCdir << "\\" << "Test.dll";
			wstr = file;//L"C:\\Users\\mike\\source\\repos\\PluginArchitectureDLL1\\x64\\Debug\\PLUGINARCHITECTUREDLL1.dll"; //ss.str();
			HMODULE dll = LoadLibrary(wstr.c_str());
			if (dll)
			{
				f_CreateModule CreateModule = (f_CreateModule)GetProcAddress(dll, "CreateModule");
				if (CreateModule)
				{
					Plugin* plugin = CreateModule();
					if (plugin)
					{

						f_GetPluginName GetPluginName = (f_GetPluginName)GetProcAddress(dll, "GetPluginName");

						f_GetListViewName GetListViewName = (f_GetListViewName)GetProcAddress(dll, "GetListViewName");
						f_GetListViewBool GetListViewBool = (f_GetListViewBool)GetProcAddress(dll, "GetListViewBool");
						f_GetImmediateActivation GetImmediateActivation = (f_GetImmediateActivation)GetProcAddress(dll, "GetListViewBool");


						plugin->PluginName = GetPluginName();
						plugin->ListViewName = GetListViewName();
						plugin->usingListView = GetListViewBool();
						plugin->activateOnLoad = GetImmediateActivation();
						plugin->PluginHandle = dll;

						if (plugin->PluginName.size() > 0)
						{
							vPlugins.push_back(plugin);

							if (plugin->usingListView)
								SendMessage(mData->HWND.ListView, WM_ADDDROPMENUITEM, (WPARAM)dll, (LPARAM)plugin->ListViewName.c_str());
							if (plugin->activateOnLoad)
								plugin->Initialize();
